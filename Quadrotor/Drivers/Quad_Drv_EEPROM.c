...
#define EE_ERROR_PG HAL_ERROR
#define EE_ERROR_WRP HAL_ERROR
#define EE_COMPLETE HAL_OK
#define EE_TIMEOUT HAL_TIMEOUT
#define EE_Status HAL_StatusTypeDef
		
																			
#define FLASH_ErasePage(__PAGE_ADDRESS__) \
(\
	EraseInit.TypeErase = FLASH_TYPEERASE_PAGES,\
	EraseInit.NbPages = 1,\
	EraseInit.PageAddress = __PAGE_ADDRESS__,\
...
{        
  EE_Status FlashStatus = EE_COMPLETE;
...
/**
  ...
  */
uint16_t Quad::EE_ReadVariable(uint16_t VirtAddress, uint16_t* Data)
{
  uint16_t ValidPage = PAGE0;
...
/**
  ...
  */
uint16_t Quad::EE_WriteVariable(uint16_t VirtAddress, uint16_t Data)
{
  uint16_t Status = 0;
...
static uint16_t EE_VerifyPageFullWriteVariable(uint16_t VirtAddress, uint16_t Data)
{
  ...
  while (CurWrAddress < PageEndAddress)
  {
    ...
    {
      /* Set variable data */
      FlashStatus = HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, CurWrAddress, Data);
      /* If program operation was failed, a Flash error code is returned */
      if (FlashStatus != EE_COMPLETE)
      {
        return FlashStatus;
      ...
      /* Set variable virtual address */
      FlashStatus = HAL_FLASH_Program(FLASH_TYPEPROGRAM_HALFWORD, CurWrAddress + 2, VirtAddress);
